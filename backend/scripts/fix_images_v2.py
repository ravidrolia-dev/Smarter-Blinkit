...
def is_valid_image(url):
    if not url or not url.startswith("http"):
        return False
    # Use a real User-Agent to avoid being blocked by Amazon/Flipkart
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
    }
    try:
        # Some sites block HEAD, use GET with stream=True and check headers
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            for p in products:
                ...
                elif "beauty" in cat.lower() or "fragrance" in cat.lower(): pools["Personal Care"].append(img)
                elif "furniture" in cat.lower() or "home" in cat.lower(): pools["Household"].append(img)
...
async def fix_images_v2():
    ...
    # Pre-fetch all products to avoid cursor timeout if verification is slow
    all_products = await cursor.to_list(length=1000)
    ...
    for product in all_products:
        ...
        # Immediate flags for broken images
        needs_fix = False
        if not url:
            needs_fix = True
        elif "flaticon.com" in url or "example.com" in url or "dummyimage.com" in url:
            needs_fix = True
        else:
            # Deep verification
            if not is_valid_image(url):
                needs_fix = True
                print(f"  [!] Detected broken image for: {name}")
        ...
        if needs_fix:
            ...
            fixed_count += 1
            print(f"  [✓] FIXED: {name} -> {new_url[:50]}...")

    print(f"\nFinal Update Job Complete!")
    print(f"Total checked: {total_checked}")
    print(f"Total fixed: {fixed_count}")

if __name__ == "__main__":
    asyncio.run(fix_images_v2())
